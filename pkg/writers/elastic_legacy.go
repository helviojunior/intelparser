// Legacy Elasticsearch writer: it reproduces the flat, pre-restructure index
// layout — one main file index plus <index>_creds/_urls/_emails/_phone/_document
// sibling indices whose documents embed file_id/bucket — kept for datasets that
// were not migrated to the file/leak/reference model implemented in elastic.go.
//
// Every package-level identifier here carries a Legacy marker so both writers
// can live in the same package; the formatting helpers (fmtInt, fmtRate,
// renderKVTable) are shared with elastic.go and intentionally not duplicated.
package writers
...
var elkLegacyExludedFields = []string{"failed", "failed_reason", "near_text"}
var elkLegacyBulkCount = 200
var elkLegacyBulkMaxSize = 5 * 1024 * 1024
var elkLegacyWorkers = 4
...
var elkLegacyCodec = "best_compression"

// legacyQueueItem wraps a File with the timestamp it was enqueued at, so workers
...
type legacyBulkResponse struct {
...
type legacyIndexResponse struct {
	ID     string `json:"_id"`
...
// NewElasticLegacyWriter returns a new Elasticsearch writer using the legacy
// index layout.
// When debug is true, operational logs are emitted at Info level; otherwise
// they are emitted at Debug level.
func NewElasticLegacyWriter(uri string, debug bool) (*ElasticLegacyWriter, error) {
	...
	if v1, ok := os.LookupEnv("ELK_TRANSLOG_DURABILITY"); ok {
		...
		elkLegacyTranslogDurability = v1
	}
	...
	//File Index
	err = wr.CreateIndex(wr.Index, buildLegacyIndexBody(`{
	...
	wr.queue = make(chan *legacyQueueItem, elkLegacyQueueSize)
	...
	logger.Infof("Starting ELK writer with %d workers (queue=%d, bulk=%d docs/%s)",
		elkLegacyWorkers, elkLegacyQueueSize, elkLegacyBulkCount, tools.Bytes(uint64(elkLegacyBulkMaxSize)))
	for i := 0; i < elkLegacyWorkers; i++ {
		wr.wg.Add(1)
		go wr.worker()
	}
	wr.reporterWG.Add(1)
...
// buildLegacyIndexBody wraps a mappings "properties" object (passed as a raw JSON
...
func (ew *ElasticLegacyWriter) logf(format string, args ...interface{}) {
	if ew.debug {
...
func (ew *ElasticLegacyWriter) Write(result *models.File) error {
	if ew.closed.Load() {
		return errors.New("ElasticLegacyWriter is closed")
	}
	...
	cp := *result
	ew.queue <- &legacyQueueItem{file: &cp, enqueuedAt: time.Now()}
	return nil
}

// worker consumes files from the queue and writes them synchronously.
func (ew *ElasticLegacyWriter) worker() {
	defer ew.wg.Done()
...
func (ew *ElasticLegacyWriter) Flush() error {
...
func (ew *ElasticLegacyWriter) recordBulk(docs int, size int, dur time.Duration) {
	ew.metBulks.Add(1)
...
func (ew *ElasticLegacyWriter) metricsReporter() {
...
// logs.
func (ew *ElasticLegacyWriter) logMetrics(final bool) {
...
func (ew *ElasticLegacyWriter) Finalize() error {
	bulks := ew.metBulks.Load()
	retries := ew.metBulkRetries.Load()
	docs := ew.metDocs.Load()
...
// legacyDocHash reproduces the per-item _id of the pre-restructure model.
// The old models.CalcHash methods were dropped when the leak/reference
// indices were introduced, so the legacy formula (file fingerprint + the
// item's own fields) is kept here to preserve document ids of legacy
// datasets across re-imports.
func legacyDocHash(item interface{}, fingerprint string) (string, error) {
	var hash string
	switch v := item.(type) {
	case models.Credential:
		models.CalcRefHash(&hash, fingerprint, v.Time, v.Rule, v.UserDomain, v.Username, v.Password, v.Url)
	case models.URL:
		models.CalcRefHash(&hash, fingerprint, v.Time, v.Url)
	case models.Email:
		models.CalcRefHash(&hash, fingerprint, v.Time, v.Email)
	case models.Phone:
		models.CalcRefHash(&hash, fingerprint, v.Time, v.Phone)
	case models.Document:
		models.CalcRefHash(&hash, fingerprint, v.Time, v.Number)
	default:
		return "", fmt.Errorf("legacy elastic writer: unsupported leak type %T", item)
	}
	return hash, nil
}

// ingestLegacyItems marshals each item, attaches the standard envelope fields, and
// ships the resulting docs to `index` in bulks respecting elkLegacyBulkCount and
// elkLegacyBulkMaxSize. Kept generic so the per-index loops in writeSync stay
// identical and can run concurrently.
func ingestLegacyItems[T any](ew *ElasticLegacyWriter, index string, items []T,
	...
	for _, it := range items {
		...
		cid, err := legacyDocHash(it, fingerprint)
		if err != nil {
			return err
		}
		...
		if len(docs) >= elkLegacyBulkCount || docsLen >= elkLegacyBulkMaxSize {
...
func (ew *ElasticLegacyWriter) writeSync(result *models.File) error {
	...
	go func() {
		defer wg.Done()
		errs[0] = ingestLegacyItems(ew, ew.Index+"_creds",
			result.Credentials, result.Fingerprint, result.Bucket)
	}()
	go func() {
		defer wg.Done()
		errs[1] = ingestLegacyItems(ew, ew.Index+"_urls",
			result.URLs, result.Fingerprint, result.Bucket)
	}()
	go func() {
		defer wg.Done()
		errs[2] = ingestLegacyItems(ew, ew.Index+"_emails",
			result.Emails, result.Fingerprint, result.Bucket)
	}()
	go func() {
		defer wg.Done()
		errs[3] = ingestLegacyItems(ew, ew.Index+"_phone",
			result.Phones, result.Fingerprint, result.Bucket)
	}()
	go func() {
		defer wg.Done()
		errs[4] = ingestLegacyItems(ew, ew.Index+"_document",
...
func (ew *ElasticLegacyWriter) CreateIndex(index string, mapping string) error {
...
func (ew *ElasticLegacyWriter) MarshalAppend(marshalled []byte, new_data map[string]interface{}) ([]byte, error) {
	...
	for k, v := range t_data {
		...
		if tools.SliceHasStr(elkLegacyExludedFields, k) {
			continue
		}
